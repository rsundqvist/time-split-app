...
class ScheduleFilterWidget:
    ...
    def select(
        self,
        *,
        limit: int | Literal["min"] | None = None,
        step: int | Literal["min"] | None = None,
    ) -> Filters:
        ...
        if limit is None:
            limit = "min" if qp.n_splits is None else qp.n_splits
        if step is None:
            step = "min" if qp.step is None else qp.step
