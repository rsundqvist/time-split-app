...
from time_fold_explorer.widgets.types import SpanType, QueryParams
...
class SpanWidget:
    ...
    Args:
        is_before: If ``True``, this widget manages the `before`-span.
    ...
    def _get_span(self, label: str, default_kind: SpanType) -> Span:
        ...
        kind: SpanType | None = st.radio(
            f"Available data ***{label}*** the fold date.",
            kinds,
